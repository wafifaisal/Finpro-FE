...
const HomePage = () => {
  ...
  }, [allProperties, userLocation, selectedCategory]);
  ...
  const propertySlides = useMemo(() => {
    ...
    if (nearestProperties.length === 0) {
      return <NoResults />;
    }
    return nearestProperties.map((property) => (
      <SwiperSlide key={property.id}>
        <PropertyCard property={property} userLocation={userLocation} />
      </SwiperSlide>
    ));
  }, [loading, nearestProperties, userLocation]);

  return (
    <div>
      <HeroSection slides={slides} />
      <div className="pt-48 mx-4 sm:mx-8 md:mx-32 my-10">
        <h2 className="text-2xl font-bold text-center mb-8">{headingText}</h2>
